...
knowledge0 = And(
    ...
    Implication(AKnight, And(AKnight, AKnave))  # If A is a knight, then A cannot be both a knave and a knight
)
...
knowledge1 = And(
    ...
    Implication(BKnave, Not(BKnight)),  # If B is a knave, then B is not a knight
    Implication(AKnight, And(AKnave, BKnave)),  # If A is a knight, then both A and B are knaves
    Implication(AKnave, Not(And(AKnave, BKnave)))  # If A is a knave, then not both A and B are knaves
)
...
knowledge3 = And(
    ...
    Implication(BKnave, Or(Not(Implication(AKnight, AKnave)), Implication(AKnave, Not(AKnave)))),
